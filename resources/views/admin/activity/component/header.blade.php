...
<form action="{{ route('admin.' . $model . '.index') }}" method="GET"
    ...
    <div class="relative w-full md:w-1/4">
        <select name="column" id="search-column"
                class="block appearance-none w-full h-12 dark:bg-gray-700 bg-blue-600 text-gray-200 border border-gray-500 rounded leading-tight focus:outline-none dark:focus:bg-gray-600 focus:bg-blue-500 focus:border-blue-500 transition duration-150"
                required>
            <option value="" disabled selected hidden>@lang('admin.choose_search_column')</option>
            @foreach($searchableColumns as $column)
                <option
                    value="{{ $column }}" {{ request('column') == $column ? 'selected' : '' }}>@lang('admin.' . $column)</option>
            @endforeach
        </select>
    </div>
    <!-- Search Input -->
