...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden p-6">
            ...
            @endif
            <form action="{{ route('admin.' . $model . '.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    @php
                        $fields = [
                            ...
                            ['name' => 'new_password', 'type' => 'password', 'label' => 'Password', 'placeholder' => 'New password (leave blank to keep current)', 'required' => false],
                            ['name' => 'referral_code', 'type' => 'text', 'label' => 'Referral Code', 'required' => false],
                            ['name' => 'role', 'type' => 'select', 'label' => 'Role', 'options' => $roles->pluck('name', 'id')->toArray(), 'current' => $currentRole ?? 'None', 'required' => true]
                        ];
                    @endphp
                                ...
                                <select id="{{ $field['name'] }}" name="{{ $field['name'] }}" class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" @if($field['required']) required @endif>
                                    @if($field['name'] == 'role')
                                        ...
                                        @foreach($roles as $role)
                                            <option value="{{ $role->uuid }}" {{ ($currentRole == $role->name) ? 'selected' : '' }}>{{ Lang::get('roles.' . $role->name) }}</option>
                                        @endforeach
                                    @else
                                        @foreach($field['options'] as $value => $text)
                                            <option value="{{ $value }}" {{ old($field['name'], $data->{$field['name']}) == $value ? 'selected' : '' }}>{{ $text }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            @elseif($field['type'] === 'file')
                                <input type="file" name="{{ $field['name'] }}" id="{{ $field['name'] }}"
                                       class="mt-1 block w-full px-3 file:border file:border-gray-300 border border-gray-300 dark:border-gray-600 rounded-md file:text-sm file:font-semibold file:bg-white file:py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 hover:text-indigo-500 dark:text-gray-200 dark:bg-gray-700" @if($field['required']) required @endif>
                            @else
                                <input type="{{ $field['type'] }}" name="{{ $field['name'] }}" id="{{ $field['name'] }}" value="{{ old($field['name'], $data->{$field['name']}) }}"
                                       class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
